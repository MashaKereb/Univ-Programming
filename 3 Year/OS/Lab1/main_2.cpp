...
//**************************************************************************************************//
 ...
 /*   // this is required for synchronization
    ...
    CalculationResult result = CalculationResult::NONE;
    int result_value = 0;
    ...
    // user input listener
    boost::thread user_interruption_thread([&is_ended, &res_mutex, &all_done]{

       bool res = checkIfExit();
       if(res){
           res_mutex.lock();
           all_done = true;
           res_mutex.unlock();
