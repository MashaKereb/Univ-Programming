...
public class AMDFileSystem implements IFileSystem {
    ...
    public void create(String filename) throws NameTooLong, OutOfMemory, FileAlreadyExists {
        if(filename.length() > FSConfig.fileNameMaxLength) throw new NameTooLong("The file name is too long");
    ...
    @Override
    public byte[] read(int index, int count) throws OutOfRange{
        File file = OFT[index];
        if(file == null) throw  new IllegalArgumentException("Illegal file index");
        ...
        count = count < fileDescriptor.fileLength - file.position ?
                count:fileDescriptor.fileLength - file.position;
        ...
        while (offset < count) {
            ...
            if (bufferPos == file.buffer.length - 1) {
                ...
                file.buffer = DataManager.readBytesByBlockIndex(
                        file.descriptorWrapper.descriptor.blocksIndices[blockNum + 1]
                );
