...
from data_type import Point, Event, Arc, Segment
...
class Voronoi:
    ...
    def process(self):
        while not self.points.empty():
            if not self.events.empty() and (self.events.top().x <= self.points.top().x):
    ...
    def process_event(self):
        ...
        if e.valid:
            ...
            if a.s0 is not None:
                a.s0.finish(e.parabola)
            if a.s1 is not None:
                a.s1.finish(e.parabola)
