package schnorr.blake2b;

import java.util.Arrays;

public class Blake2bBuilder {
    private static Blake2BTreeConfig sequentialTreeConfig =
            new Blake2BTreeConfig(0, 0, 1, 1);
    ...
    public static long[] configB(Blake2bConfig config, Blake2BTreeConfig treeConfig) throws Exception {
        ...
        if (config.getKey() != null) {
            if (config.getKey().length > 64)
                throw new Exception(Arrays.toString(config.getKey()) + "Key too long");
